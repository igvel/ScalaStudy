...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors filter { case (b, h) => !explored.contains(b) }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    val more = for {
      (b1, h1) <- initial
      (b2, h2) <- newNeighborsOnly(neighborsWithHistory(b1, h1), explored)
    } yield (b2, h2)
    if(more.isEmpty) initial
    else initial append from(more, explored ++ more.map { case (b, _) => b }.toSet)
  }
